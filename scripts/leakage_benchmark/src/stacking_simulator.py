    ...
    try:
        ...
        if plot_insights:
            ...
            l2_test_oof = pd.DataFrame(
                np.array([predictor.predict_proba(l2_test_data, model=m, as_multiclass=False, as_pandas=False) for m in predictor.get_model_names()]).T,
                columns=predictor.get_model_names(),
            )

            f_cols = [x for x in l2_train_data.columns if x not in oof_col_names + [label]]
            get_proba_insights(
                l2_train_data.loc[:, oof_col_names],
                l2_test_data.loc[:, oof_col_names],
                l2_train_oof,
                l2_test_oof,
                l2_train_data[label],
                l2_test_data[label],
                l2_train_data.loc[:, f_cols],
                l2_test_data.loc[:, f_cols],
                eval_metric,
                predictor,
            )
