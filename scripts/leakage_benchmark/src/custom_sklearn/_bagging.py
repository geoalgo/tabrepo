"""Bagging meta-estimator from sklearn adapted to support dropout."""
...
def _parallel_build_estimators(
    ...
    for i in range(n_estimators):
        ...
        X_m = X.copy()  # maybe not needed due to slice below but to be sure.
        # Draw samples, using sample weights, and then fit
        if support_sample_weight and False:  # to make dropout work as intended for now disable this
            ...
            X_m, y_m, sample_weight_m = X_m[:, features], y, curr_sample_weight
            raise NotImplementedError("only implemented for no sample_weight so far")
        else:
            X_m, y_m, sample_weight_m =X_m[indices][:, features], y[indices], None

        if dropout:
            rng = np.random.RandomState(random_state)
            positive_class_proba = (y_m == np.unique(y_m)[1]).astype(int)
            potential_sample = np.arange(X_m.shape[0])
            dropout_chance = dropout
            for f in features:
                i_sel = rng.choice(potential_sample, int(np.floor(potential_sample.shape[0] * dropout_chance)), replace=False)
                tmp_vals = positive_class_proba[i_sel].copy()
                tmp_vals[tmp_vals == 0] = np.min(X_m[f])
                tmp_vals[tmp_vals == 1] = np.max(X_m[f])
                X_m[i_sel, f] = tmp_vals

        estimator_fit(X_m, y_m, sample_weight=sample_weight_m)
