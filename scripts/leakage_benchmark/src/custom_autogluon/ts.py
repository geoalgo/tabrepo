...
def tune_temperature_scaling(y_val_probs: np.ndarray, y_val: np.ndarray, init_val: float = 1, max_iter: int = 1000,
                             lr: float = 0.01, sample_weight: np.ndarray = None):
    """
    ...
    nll_criterion = torch.nn.CrossEntropyLoss(reduction='none')
...
if __name__ == "__main__":
    # Quick test for sample weights
    s = tune_temperature_scaling(
        y_val_probs=np.array([[0.9, 0.2], [0.6, 0.4], [0.5, 0.5], [0.7, 0.3], [0.2, 0.8], [0.25, 0.75]]),
    ...
    s = tune_temperature_scaling(
        y_val_probs=np.array([[0.9, 0.2], [0.6, 0.4], [0.5, 0.5], [0.7, 0.3], [0.2, 0.8], [0.25, 0.75]]),
        y_val=np.array([0, 0, 1, 0, 1, 1]),
        sample_weight=np.array([1, 5, 1, 2, 10, 1])
    )
    print(s)